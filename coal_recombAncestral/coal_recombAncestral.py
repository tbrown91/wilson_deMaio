...
def main():
    ...
    while remaining_nodes > 1:
        ...
        current_time = current_time - 2 * np.log(1 - np.random.rand(1)) / (remaining_nodes * (remaining_nodes - 1 + recomb_rate))

        #Determine whether event is coalescent or recombination
        reac_rand = np.random.rand(1)
        if reac_rand[0] < (float(remaining_nodes - 1) / (remaining_nodes - 1 + recomb_rate)):
            ...
            if dot_check == 1:
                ...
                DOT_coal(node_list[new_node].ancestral,new_node,children,node_list[children[0]].ancestral,node_list[children[1]].ancestral)
            ...
            #Choose break point
            break_point = random.sample(range(genome_length),1)
            break_point = break_point[0]

            #Check if break point splits up ancestral material
            left_break = 0
            right_break = 0
            if len(node_list[child[0]].ancestral_material) == 0:
                print (str(child[0]) + " has no ancestral material\n")
                break

            for i in range(len(node_list[child[0]].ancestral_material)):
                if (break_point > node_list[child[0]].ancestral_material[i][1]):
                    left_break = left_break + 1
                elif (break_point <= node_list[child[0]].ancestral_material[i][0]):
                    right_break = right_break + 1

            #If recombinant break does not split up ancestral material, ignore recombination event
            if (left_break != len(node_list[child[0]].ancestral_material)) and (right_break != len(node_list[child[0]].ancestral_material)):
                ...
                [node_list[new_node].ancestral_material,node_list[new_node+1].ancestral_material] = split_ancestry(node_list[child[0]].ancestral_material,break_point)
                ...
                if dot_check == 1:
                    ...
                    DOT_recomb(new_node,new_node+1,child[0],node_list[child[0]].ancestral,node_list[new_node].ancestral,node_list[new_node+1].ancestral)
